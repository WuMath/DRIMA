...
def evaluate_model_link_prediction(model_name: str, 
                                   model: nn.Module,
    ...
    # (i.e. we always sample the same negatives for validation / test set)
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            negative_probabilities = model[1](input_1=batch_neg_src_node_embeddings, 
                                              ...
                                              input_3=batch_neg_time_diff_embeddings).squeeze(dim=-1).sigmoid()
            ...
            with torch.no_grad():
                    y_scores = torch.sigmoid(predicts)

            evaluate_metrics.append(get_link_prediction_metrics(predicts=y_scores, labels=labels))
