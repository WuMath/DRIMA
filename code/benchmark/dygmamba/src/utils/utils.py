...
class NeighborSampler:
    ...
    def __init__(self, adj_list: list, sample_neighbor_strategy: str = 'uniform', 
                time_scaling_factor: float = 0.0, seed: int = None, max_neighbors: int = None):
        """
        Neighbor sampler.
        ...
        :param seed: int, random seed
        :param max_neighbors: int, maximum number of neighbors to sample (None means no limit)  # 新增文档
        """
    ...
    def find_neighbors_before(self, node_id: int, interact_time: float, return_sampled_probabilities: bool = False):
        ...
        # ↓ 新增这一行：直接从源头截断，避免后续处理大量无用邻居
        if self.max_neighbors is not None:
            i_start = max(0, i - self.max_neighbors)
        else:
            i_start = 0
        
        if return_sampled_probabilities:
            return self.nodes_neighbor_ids[node_id][i_start:i], \
                self.nodes_edge_ids[node_id][i_start:i], \
                self.nodes_neighbor_times[node_id][i_start:i], \
                self.nodes_neighbor_sampled_probabilities[node_id][i_start:i]
        else:
            return self.nodes_neighbor_ids[node_id][i_start:i], \
                self.nodes_edge_ids[node_id][i_start:i], \
                self.nodes_neighbor_times[node_id][i_start:i], None
    ...
    def get_historical_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 20):
        ...
        # ndarray, shape (batch_size, num_neighbors)
        nodes_neighbor_times = np.zeros((len(node_ids), num_neighbors)).astype(np.float64)
    ...
    def get_all_first_hop_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray,
                                    interact_dst_ids: np.ndarray = None,
                                    interact_edge_ids: np.ndarray = None):
        """
        ...
        :param node_ids: ndarray, shape (batch_size, ), node ids
        :param node_interact_times: ndarray, shape (batch_size, ), node interaction times
        :param interact_dst_ids: ndarray, shape (batch_size, ), 当前时刻与 node_ids[i] 交互的对方节点id
                        当历史邻居为空时，用此节点作为填充，避免全零导致NaN
        :param interact_edge_ids: ndarray, shape (batch_size, ), 当前时刻交互对应的边id
                        与 interact_dst_ids 配套使用
        :return:
        """
        ...
        for idx, (node_id, node_interact_time) in enumerate(zip(node_ids, node_interact_times)):
            ...
            #####################################
            # 改
                        # 【修复】历史邻居为空（节点首次出现）：用当前时刻的交互对方节点填充
            if len(node_neighbor_ids) == 0:
                if interact_dst_ids is not None:
                    fill_node = interact_dst_ids[idx]
                    if hasattr(fill_node, 'cpu'):
                        fill_node = int(fill_node.cpu().item())
                    else:
                        fill_node = int(fill_node)
                    fill_edge = 0
                    if interact_edge_ids is not None:
                        fill_edge = interact_edge_ids[idx]
                        if hasattr(fill_edge, 'cpu'):
                            fill_edge = int(fill_edge.cpu().item())
                        else:
                            fill_edge = int(fill_edge)
                    node_neighbor_ids  = np.array([fill_node],        dtype=np.longlong)
                    node_edge_ids      = np.array([fill_edge],        dtype=np.longlong)
                    node_neighbor_times = np.array([node_interact_time], dtype=np.float32)
                else:
                    # 没有提供对方节点信息时，用自身节点作为兜底
                    node_neighbor_ids  = np.array([node_id],           dtype=np.longlong)
                    node_edge_ids      = np.array([0],                 dtype=np.longlong)
                    node_neighbor_times = np.array([node_interact_time], dtype=np.float32)
            #####################################
        ...
        # new revise
        if self.max_neighbors is not None:
            limited_neighbor_ids = []
            limited_edge_ids = []
            limited_neighbor_times = []
            
            # 遍历每个节点的邻居列表
            for neighbors, edges, times in zip(nodes_neighbor_ids_list, 
                                            nodes_edge_ids_list, 
                                            nodes_neighbor_times_list):
                # 如果邻居数超过限制
                if len(neighbors) > self.max_neighbors:
                    # 按时间排序，选择最近的K个邻居
                    indices = np.argsort(times)[-self.max_neighbors:]
                    neighbors = neighbors[indices]
                    edges = edges[indices]
                    times = times[indices]
                
                # 保存（限制后的或未超限的）邻居
                limited_neighbor_ids.append(neighbors)
                limited_edge_ids.append(edges)
                limited_neighbor_times.append(times)
        
        # 用限制后的列表替换原列表
        nodes_neighbor_ids_list = limited_neighbor_ids
        nodes_edge_ids_list = limited_edge_ids
        nodes_neighbor_times_list = limited_neighbor_times
...
def get_neighbor_sampler(data: Data, sample_neighbor_strategy: str = 'uniform', 
    ...
    a large time_scaling_factor tends to sample more on recent links, this parameter works when sample_neighbor_strategy == 'time_interval_aware'
    :param seed: int, random seed
    :param max_neighbors: int, maximum number of neighbors to return (None means no limit)  # 新增文档
    :return:
    """
    ...
    for src_node_id, dst_node_id, edge_id, node_interact_time in zip(data.src_node_ids, 
                            data.dst_node_ids, data.edge_ids, 
    ...
    return NeighborSampler(adj_list=adj_list, sample_neighbor_strategy=sample_neighbor_strategy, 
                            time_scaling_factor=time_scaling_factor, seed=seed, max_neighbors=max_neighbors)
...
class NegativeEdgeSampler(object):
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, interact_times: np.ndarray = None, 
        ...
        if self.negative_sample_strategy != 'random':
            ...
            self.possible_edges = set((src_node_id, dst_node_id) for src_node_id in self.unique_src_node_ids 
