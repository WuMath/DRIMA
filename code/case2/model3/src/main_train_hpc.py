...
def load_link_prediction_args(is_evaluation: bool = False):
    """
    ...
    parser.add_argument('--sample_neighbor_strategy', type=str, default='uniform', help='how to sample historical neighbors')
    parser.add_argument('--time_scaling_factor', default=1e-6, type=float, help='the hyperparameter that controls the sampling preference with time interval, a large time_scaling_factor tends to sample more on recent links, 0.0 corresponds to uniform sampling, it works when sample_neighbor_strategy == time_interval_aware')
    ...
    parser.add_argument('--num_epochs', type=int, default=1, help='number of epochs') # default 100
    
    parser.add_argument('--optimizer', type=str, default='Adam', help='name of optimizer')
    ...
    parser.add_argument('--negative_sample_strategy', type=str, default='random', help='strategy for the negative edge sampling')
...
if __name__ == "__main__":
    ...
    Node_feature = NodeFeatureLookup.load(
        data_path + "node_feat_matrix.dat",
        data_path + "node_feat_meta.pkl"
    )
