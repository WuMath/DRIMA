...
def analysis_tf_recovery(data_path, output_path, method_list):
    ###########################################################
...
def analysis_tf_gene_data(data_path, method_list):
    ...
    if "DyGMamba" in method_list:
        ...
        result_dict.update({"Active": active_result, "Global": global_result})
    ...
    if "GLUE" in method_list:
        ...
        result_dict.update({"GLUE": glue_result})
    ...
    if "FigR" in method_list:
        ...
        result_dict.update({"FigR": figr_result})
    ...
    if "CellOracle" in method_list:
        ...
        result_dict.update({"CellOracle": celloracle_result})
    ...
    if "LINGER" in method_list:
        ...
        result_dict.update({"LINGER": linger_result})
    ...
    if "GRaNIE" in method_list:
        ...
        result_dict.update({"GRaNIE": granie_result})
    ...
    if "Pando" in method_list:
        ...
        result_dict.update({"Pando": pando_result})
...
def analysis_tf_gene_all_correlation(output_path, result_dict, method_colors):
    ...
    for method_name, df in result_dict.items():
        ...
        new_labels_dict.update({method_name: f"{method_name} (n={len(df)})"})
...
def analysis_tf_gene_all_precision(data_path, output_path, 
                                   method_list, method_colors, beta = 1):
...
def analysis_tf_gene_per_tf_precision(data_path, output_path, method_list, beta=1.0):
    """
    ...
    threshold = benchmark_tf_gene_df["value"].quantile(0.25)
    benchmark_tf_gene_df["label"] = (benchmark_tf_gene_df["value"] > threshold).astype(int)
    benchmark_tf_gene_df.drop_duplicates(subset=['TF', 'Gene'], inplace=True)

    # ── 各方法 GRN 加载（与 analysis_tf_gene_all_precision 相同逻辑）────────
    method_grn_dict = {}

    if "DyGMamba" in method_list:
        dyg_result_path = data_path + "data_dyg/"
        avg_active = pd.read_pickle(dyg_result_path + "average_active_tf_gene_grn.pkl")
        dyg_grn = avg_active[["TF", "Gene", "avg_total_weight"]].copy()
        dyg_grn["predict_label"] = (dyg_grn["avg_total_weight"] > 1).astype(int)
        dyg_grn.drop_duplicates(subset=['TF', 'Gene'], inplace=True)
        method_grn_dict["DyGMamba"] = dyg_grn

    if "GLUE" in method_list:
        glue_df = glue_read_full_grn(data_path + "data_glue/pruned_grn.csv")
        glue_grn = glue_df[["TF", "Target", "Importance"]].rename(columns={"Target": "Gene"})
        glue_grn["predict_label"] = (glue_grn["Importance"] > 0).astype(int)
        glue_grn.drop_duplicates(subset=['TF', 'Gene'], inplace=True)
        method_grn_dict["GLUE"] = glue_grn

    if "FigR" in method_list:
        figr_df = pd.read_csv(data_path + "data_FigR/TF_Gene_Network.csv")
        figr_grn = figr_df[["TF", "Target_Gene", "Correlation"]].rename(
            columns={"Target_Gene": "Gene", "Correlation": "score"})
        figr_grn["score"] = figr_grn["score"].abs()
        figr_grn["predict_label"] = (figr_grn["score"] > 0).astype(int)
        figr_grn.drop_duplicates(subset=['TF', 'Gene'], inplace=True)
        method_grn_dict["FigR"] = figr_grn

    if "CellOracle" in method_list:
        co_df = pd.read_csv(data_path + "data_celloracle/celloracle_results/grn_df_cluster0.csv")
        co_df.rename(columns={"source": "TF", "target": "Gene"}, inplace=True)
        co_grn = co_df[["TF", "Gene", "coef_abs"]].rename(columns={"coef_abs": "score"})
        co_grn["predict_label"] = (co_grn["score"] > 0).astype(int)
        co_grn.drop_duplicates(subset=['TF', 'Gene'], inplace=True)
        method_grn_dict["CellOracle"] = co_grn

    if "LINGER" in method_list:
        li_df = pd.read_csv(data_path + "data_linger/train/cell_population_trans_regulatory.txt", sep='\t')
        li_df = li_df.rename(columns={'Unnamed: 0': 'Gene'})
        li_df = li_df.melt(id_vars=['Gene'], var_name='TF', value_name='score')
        quantile_val = li_df["score"].quantile(0.25)
        li_df["predict_label"] = (li_df["score"] > quantile_val).astype(int)
        li_grn = li_df[["TF", "Gene", "predict_label"]].drop_duplicates(subset=['TF', 'Gene'])
        method_grn_dict["LINGER"] = li_grn

    if "GRaNIE" in method_list:
        gr_df = pd.read_csv(data_path + "GRaNIE_output/GRaNIE_tf_gene_Links.csv")
        gr_df.dropna(inplace=True)
        gr_df["score"] = gr_df["TF_peak.r"].abs() * gr_df["peak_gene.r"].abs()
        gr_grn = gr_df[["TF.name", "gene.name", "score"]].rename(
            columns={"TF.name": "TF", "gene.name": "Gene"})
        q = gr_grn["score"].quantile(0.25)
        gr_grn["predict_label"] = (gr_grn["score"] > q).astype(int)
        gr_grn.drop_duplicates(subset=['TF', 'Gene'], inplace=True)
        method_grn_dict["GRaNIE"] = gr_grn

    if "Pando" in method_list:
        pa_df = pd.read_csv(data_path + "data_pando/tf_gene_network.csv")
        pa_df["predict_label"] = (pa_df["padj"] < 0.04).astype(int)
        pa_grn = pa_df[["TF", "Gene", "predict_label"]].drop_duplicates(subset=['TF', 'Gene'])
        method_grn_dict["Pando"] = pa_grn
    ...
    for method_name, pred_df in method_grn_dict.items():
        per_tf = _compute_per_tf_metrics(benchmark_tf_gene_df, pred_df, beta=beta)
        per_tf["Method"] = method_name
        ...
        print(f"[TF-Gene per-TF] {method_name}: "
            f"median Precision={per_tf['Precision'].median():.4f}, "
            f"median F{beta}={per_tf['F_score'].median():.4f}, "
            f"n_TF={len(per_tf)}")
    ...
    ax = sns.boxplot(data=prec_combined, x="Method", y="Precision",
                    order=method_order_prec, palette="Set2", width=0.55, showfliers=False)
    ...
    ax = sns.boxplot(data=fscore_combined, x="Method", y="F_score",
                    order=method_order_f, palette="Set2", width=0.55, showfliers=False)
...
def analysis_tf_gene_correlation_summary(result_dict, output_path):
    """
...
def analysis_tf_region_overall(tf_region_result, output_path, beta=1.0):
    """
    ...
    for col, title, ax in metrics:
        ...
        bp = sns.barplot(data=sorted_df, x="Method", y=col, palette="viridis", ax=ax)
    ...
    ax = sns.barplot(data=plot_df, x="Method", y="Score",
                    hue="Metric", palette=["#5DCAA5", "#EF9F27"])
...
def analysis_region_gene_precision(benchmark_peak_gene_df, method_data, output_path, beta=1.0):
    """
...
def analysis_tf_gene_per_tf_topn(
    data_path,
    output_path,
    method_list,
    ...
    Parameters
    ----------
    data_path   : str   数据根目录（与 hic_jaspar_main.py 中保持一致）
    output_path : str   图片输出目录
    method_list : list  参与对比的方法名列表
    ...
    # ── 金标准：JASPAR TF-Gene network（25th 百分位阈值）────────────────────
    benchmark_grn = ad.read_h5ad(data_path + "process/tf_gene_network.h5ad")
    benchmark_df  = adata_to_dataframe(benchmark_grn)
    benchmark_df  = benchmark_df.rename(columns={"obs": "TF", "var": "Gene"})

    threshold = benchmark_df["value"].quantile(0.25)
    benchmark_df["label"] = (benchmark_df["value"] > threshold).astype(int)
    benchmark_df.drop_duplicates(subset=["TF", "Gene"], inplace=True)

    # ── 各方法 GRN 加载（路径与 analysis_tf_gene_all_precision 完全一致）────
    method_grn_dict = {}

    if "DyGMamba" in method_list:
        df = pd.read_pickle(data_path + "data_dyg/average_active_tf_gene_grn.pkl")
        df = df[["TF", "Gene", "avg_total_weight"]].copy()
        df["predict_label"] = (df["avg_total_weight"] > 1).astype(int)
        df.drop_duplicates(subset=["TF", "Gene"], inplace=True)
        method_grn_dict["DyGMamba"] = df

    if "GLUE" in method_list:
        df = glue_read_full_grn(data_path + "data_glue/pruned_grn.csv")
        df = df[["TF", "Target", "Importance"]].rename(columns={"Target": "Gene"})
        df["predict_label"] = (df["Importance"] > 0).astype(int)
        df.drop_duplicates(subset=["TF", "Gene"], inplace=True)
        method_grn_dict["GLUE"] = df

    if "FigR" in method_list:
        df = pd.read_csv(data_path + "data_FigR/TF_Gene_Network.csv")
        df = df[["TF", "Target_Gene", "Correlation"]].rename(
            columns={"Target_Gene": "Gene", "Correlation": "score"})
        df["score"] = df["score"].abs()
        df["predict_label"] = (df["score"] > 0).astype(int)
        df.drop_duplicates(subset=["TF", "Gene"], inplace=True)
        method_grn_dict["FigR"] = df

    if "CellOracle" in method_list:
        df = pd.read_csv(data_path + "data_celloracle/celloracle_results/grn_df_cluster0.csv")
        df.rename(columns={"source": "TF", "target": "Gene"}, inplace=True)
        df = df[["TF", "Gene", "coef_abs"]].rename(columns={"coef_abs": "score"})
        df["predict_label"] = (df["score"] > 0).astype(int)
        df.drop_duplicates(subset=["TF", "Gene"], inplace=True)
        method_grn_dict["CellOracle"] = df

    if "LINGER" in method_list:
        df = pd.read_csv(
            data_path + "data_linger/train/cell_population_trans_regulatory.txt", sep="\t")
        df = df.rename(columns={"Unnamed: 0": "Gene"})
        df = df.melt(id_vars=["Gene"], var_name="TF", value_name="score")
        q  = df["score"].quantile(0.25)
        df["predict_label"] = (df["score"] > q).astype(int)
        df = df[["TF", "Gene", "predict_label"]].drop_duplicates(subset=["TF", "Gene"])
        method_grn_dict["LINGER"] = df

    if "GRaNIE" in method_list:
        df = pd.read_csv(data_path + "GRaNIE_output/GRaNIE_tf_gene_Links.csv")
        df.dropna(inplace=True)
        df["score"] = df["TF_peak.r"].abs() * df["peak_gene.r"].abs()
        df = df[["TF.name", "gene.name", "score"]].rename(
            columns={"TF.name": "TF", "gene.name": "Gene"})
        q  = df["score"].quantile(0.25)
        df["predict_label"] = (df["score"] > q).astype(int)
        df.drop_duplicates(subset=["TF", "Gene"], inplace=True)
        method_grn_dict["GRaNIE"] = df

    if "Pando" in method_list:
        df = pd.read_csv(data_path + "data_pando/tf_gene_network.csv")
        df["predict_label"] = (df["padj"] < 0.04).astype(int)
        df = df[["TF", "Gene", "predict_label"]].drop_duplicates(subset=["TF", "Gene"])
        method_grn_dict["Pando"] = df

    # ── 逐方法计算每 TF 的 Precision / Recall / F_score ─────────────────────
    per_tf_dict = {}
    ...
    for method_name, pred_df in method_grn_dict.items():
        ...
        for tf, grp in merged.groupby("TF"):
            ...
            records.append({"TF": tf, "Precision": prec,
                            "Recall": rec, "F_score": fscore})

        per_tf_df = pd.DataFrame(records)
        per_tf_dict[method_name] = per_tf_df

        label_f = f"F{beta}" if beta != 1.0 else "F1"
        print(f"[Per-TF] {method_name:12s} | "
            f"n_TF={len(per_tf_df):4d} | "
            f"median Prec={per_tf_df['Precision'].median():.4f} | "
            f"median {label_f}={per_tf_df['F_score'].median():.4f}")
