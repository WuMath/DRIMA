...
def get_graph_node_feature(adata_rna, adata_atac, cell_pseudotime, node_id):
    ...
    修复: 原版 toarray() 在 BMMC(69k cells × 100k peaks) 上需要 ~55 GB，会被 OOM Kill。
          改为 CSC 稀疏格式，列访问 O(nnz_col)，内存只占原始稀疏大小（~几百 MB）。
    """
    import sys
    time_values = cell_pseudotime.iloc[:, 0].values
 
    print("Processing Node Features (memory-safe CSC)...", flush=True)
 
    # ---- 关键修复: 保持稀疏，转为 CSC 支持高效列访问，避免 toarray() OOM ----
    X_rna  = sp_sparse.csc_matrix(adata_rna.X)   # (n_cells, n_genes)
    X_atac = sp_sparse.csc_matrix(adata_atac.X)  # (n_cells, n_peaks)
    ...
    for i in tqdm(range(len(node_id)), file=sys.stdout):
        node_idx  = i + 1
        node_name = node_id.iloc[i]["name"]
        node_type = node_id.iloc[i]["type"]
        ...
        if node_type == "gene":
            if node_name not in rna_var_set:
                continue
            col = X_rna.getcol(rna_col_idx[node_name])
            feature_col = np.asarray(col.todense()).flatten()
        else:
            if node_name not in atac_var_set:
                continue
            col = X_atac.getcol(atac_col_idx[node_name])
            feature_col = np.asarray(col.todense()).flatten()
...
class NodeFeatureLookup:
    """
    替代 flat Python dict {(node_idx, time_val): scalar} 的轻量 wrapper。
 
    内部用 numpy memmap 2D 矩阵 (n_nodes+1, n_cells) 存储，
    对外完全保持原来的 .get((node_idx, time_val), default) 接口，
    DyGMamba.py / DataLoader.py 无需任何修改。
 
    存储对比:
      原方案: 6.5 billion Python dict entries → ~17 GB pickle，序列化卡死
      新方案: float32 memmap + 小型 time→col 映射 → ~2.5 GB，按需读页，不占 RAM
    """
 
    def __init__(self, feat_matrix, time_to_col: dict):
        """
        feat_matrix : np.ndarray 或 np.memmap, shape (n_nodes+1, n_cells), float32
                      第 0 行未使用（node_idx 从 1 开始）
        time_to_col : dict {float(pseudotime) → int(col_index)}
        """
        self.matrix = feat_matrix
        self.time_to_col = time_to_col
 
    def get(self, key, default=0.0):
        node_idx, time_val = key
        col = self.time_to_col.get(float(time_val))
        if col is None:
            return default
        row = int(node_idx)
        if row <= 0 or row >= self.matrix.shape[0]:
            return default
        return float(self.matrix[row, col])
 
    def __contains__(self, key):
        node_idx, time_val = key
        return (float(time_val) in self.time_to_col
                and 0 < int(node_idx) < self.matrix.shape[0])
 
    def save(self, matrix_path: str, meta_path: str):
        """将 memmap 矩阵和 time→col 映射持久化到磁盘"""
        import pickle as _pkl
        # 矩阵已经是 memmap，flush 即可
        if isinstance(self.matrix, np.memmap):
            self.matrix.flush()
        else:
            # 普通 ndarray → 写成 memmap
            mm = np.memmap(matrix_path, dtype='float32', mode='w+',
                           shape=self.matrix.shape)
            mm[:] = self.matrix[:]
            mm.flush()
            del mm
        with open(meta_path, 'wb') as f:
            _pkl.dump({'time_to_col': self.time_to_col,
                       'shape': self.matrix.shape}, f)
        print(f"NodeFeatureLookup saved → {matrix_path} + {meta_path}", flush=True)
 
    @classmethod
    def load(cls, matrix_path: str, meta_path: str):
        """从磁盘加载（memmap 模式，不占 RAM）"""
        import pickle as _pkl
        with open(meta_path, 'rb') as f:
            meta = _pkl.load(f)
        mm = np.memmap(matrix_path, dtype='float32', mode='r',
                       shape=tuple(meta['shape']))
        return cls(mm, meta['time_to_col'])
 

def get_graph_node_feature_new(adata_rna, adata_atac, cell_pseudotime, node_id,
                            matrix_path: str = None, meta_path: str = None):
    """
    返回 NodeFeatureLookup 对象，接口与原 dict 完全兼容。
 
    存储策略:
      若提供 matrix_path / meta_path，结果直接写到 memmap 文件，
      内存峰值仅为单列数据（~几 MB），适合 BMMC 规模。
    """
    import sys
 
    time_values = cell_pseudotime.iloc[:, 0].values          # (n_cells,)
    unique_times = np.array(sorted(set(time_values.tolist())), dtype=np.float64)
    time_to_col  = {float(t): int(i) for i, t in enumerate(unique_times)}
    # 每个 cell 对应的列下标
    cell_col_idx = np.array([time_to_col[float(t)] for t in time_values], dtype=np.int32)
 
    n_nodes   = len(node_id)
    n_timecols = len(unique_times)
 
    print(f"Node feature matrix shape: ({n_nodes + 1}, {n_timecols})", flush=True)
    print(f"  = {(n_nodes + 1) * n_timecols * 4 / 1e9:.2f} GB (float32, memmap)", flush=True)
 
    # ---- 初始化 memmap 或普通 ndarray ----
    if matrix_path is not None:
        feat_matrix = np.memmap(matrix_path, dtype='float32', mode='w+',
                                shape=(n_nodes + 1, n_timecols))
    else:
        feat_matrix = np.zeros((n_nodes + 1, n_timecols), dtype=np.float32)
 
    # ---- CSC 稀疏列访问，内存安全 ----
    X_rna  = sp_sparse.csc_matrix(adata_rna.X)
    X_atac = sp_sparse.csc_matrix(adata_atac.X)
    ...
    print("Building node feature matrix (CSC column access)...", flush=True)
 
    for i in tqdm(range(n_nodes), file=sys.stdout):
        node_idx  = i + 1                        # 1-based
        node_name = node_id.iloc[i]["name"]
        node_type = node_id.iloc[i]["type"]
        ...
        if node_type == "gene":
            if node_name not in rna_var_set:
                continue
            col_data = X_rna.getcol(rna_col_idx[node_name])
            expr_vec = np.asarray(col_data.todense()).flatten().astype(np.float32)
        else:
            if node_name not in atac_var_set:
                continue
            col_data = X_atac.getcol(atac_col_idx[node_name])
            expr_vec = np.asarray(col_data.todense()).flatten().astype(np.float32)
 
        # 把 n_cells 个标量分散写入对应的 time 列
        # cell_col_idx[j] 是第 j 个细胞对应的列下标
        # 同一个 time 列可能有多个细胞：取平均（pseudotime 通常唯一，一般无冲突）
        np.add.at(feat_matrix[node_idx], cell_col_idx, expr_vec)
 
    # 如果有 time 列碰撞（多个细胞同 pseudotime），做平均
    # 统计每列有多少细胞
    col_counts = np.bincount(cell_col_idx, minlength=n_timecols).astype(np.float32)
    col_counts[col_counts == 0] = 1.0
    feat_matrix[1:] /= col_counts[np.newaxis, :]
 
    lookup = NodeFeatureLookup(feat_matrix, time_to_col)
 
    if matrix_path is not None and meta_path is not None:
        lookup.save(matrix_path, meta_path)
...
def Graph_data_optimized(adata_rna, adata_atac, cell_pseudotime,
    ...
    """
    优化要点：
    1. rna/atac 保持稀疏 CSC 格式做列访问，避免 toarray() 的天文内存占用
       (69k cells × 100k peaks × 8 bytes ≈ 55 GB，必须避免)
    2. rp 矩阵预转 CSR，直接用行下标访问，不再做 AnnData 行切片
    3. tqdm 写入 sys.stdout，确保 Slurm --output 可见
    4. 消除内层 for k 循环：用 numpy 批量构建 edge records
    """
    import sys
 
    print("Pre-loading expression matrices as CSC sparse (memory-safe)...", flush=True)
 
    # ---- 关键修复1: 保持稀疏，转为 CSC 以支持高效列访问 O(nnz_col) ----
    # 原来的 toarray() 在 BMMC(69k cells × 100k peaks) 上需要 ~55 GB，会被 OOM Kill
    X_rna  = sp_sparse.csc_matrix(adata_rna.X)   # (n_cells, n_genes)  稀疏 CSC
    X_atac = sp_sparse.csc_matrix(adata_atac.X)  # (n_cells, n_peaks)  稀疏 CSC
    ...
    # ---- 关键修复2: rp 矩阵预转 CSR，直接用行整数下标访问，不再做 AnnData 行切片 ----
    rp_gp_csr = sp_sparse.csr_matrix(adata_rp_gene_peak.X)   # (n_genes, n_peaks)
    rp_pp_csr = sp_sparse.csr_matrix(adata_rp_peak.X)        # (n_peaks, n_peaks)
 
    # 建立 name → 行下标映射（用于 rp 矩阵）
    gp_row_idx = {name: i for i, name in enumerate(adata_rp_gene_peak.obs_names)}
    pp_row_idx = {name: i for i, name in enumerate(adata_rp_peak.obs_names)}
 
    name_to_node_idx = {name: idx + 1 for idx, name in enumerate(node_id['name'])}
    time_values = cell_pseudotime.iloc[:, 0].values   # (n_cells,)
 
    print("Processing Edges (optimized)...", flush=True)
    ...
    def _col_to_dense(csc_mat, col_i):
        """从 CSC 稀疏矩阵取第 col_i 列，返回 (n_cells,) float32 dense array"""
        col = csc_mat.getcol(col_i)
        return np.asarray(col.todense()).flatten().astype(np.float32)
 
    def _get_rp_row(csr_mat, row_i, var_names):
        """从 CSR 矩阵取第 row_i 行的非零列下标和对应值"""
        row = csr_mat.getrow(row_i)
        nz_cols = row.indices
        nz_vals = row.data
        names   = var_names[nz_cols]
        return names, nz_vals
 
    # ---- 关键修复3: tqdm 写 stdout，Slurm --output 才能捕获 ----
    for i in tqdm(range(len(node_id)), file=sys.stdout):
        current_type = node_id.iloc[i, 1]
        current_name = node_id.iloc[i, 0]
        ...
        if current_type == "gene":
            ...
            if current_name not in rna_var_set:
                continue
            row_i = gp_row_idx.get(current_name)
            if row_i is None:
                continue
 
            # ---- 修复2: 直接用 CSR 行下标取 rp 行，无需 AnnData 切片 ----
            target_peak_names, rp_values = _get_rp_row(rp_gp_csr, row_i,
                                                        adata_rp_gene_peak.var_names)
            if len(target_peak_names) == 0:
                continue
            ...
            # ---- 修复1: CSC 列访问，内存安全 ----
            tgt_act_vec = _col_to_dense(X_rna, rna_col[current_name])   # (n_cells,)
            ...
            for src_peak, tmp_RP in zip(target_peak_names, rp_values):
                ...
                src_act_vec = _col_to_dense(X_atac, atac_col[src_peak])  # (n_cells,)
                mask = src_act_vec > 0
                if not np.any(mask):
                    continue
        ...
        # ==================== CASE 2: Peak Node ====================
            ...
            if current_name not in atac_var_set:
                continue
            row_i = pp_row_idx.get(current_name)
            if row_i is None:
                continue
 
            # ---- 修复2: 直接用 CSR 行下标取 rp 行 ----
            target_peak_names, rp_values = _get_rp_row(rp_pp_csr, row_i,
                                                        adata_rp_peak.var_names)
            if len(target_peak_names) == 0:
                continue
            ...
            # ---- 修复1: CSC 列访问 ----
            tgt_act_vec = _col_to_dense(X_atac, atac_col[current_name])
            ...
            for src_peak, tmp_RP in zip(target_peak_names, rp_values):
                ...
                src_act_vec = _col_to_dense(X_atac, atac_col[src_peak])
                mask = (src_act_vec > 0) & (tgt_act_vec > 0)
                if not np.any(mask):
                    continue
